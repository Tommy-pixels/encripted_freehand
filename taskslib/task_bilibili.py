...
def run_bilibili(setting):
    ...
    dbOperator = DB_Singleton_DEFAULT()    # 获取未上传的数据
    param2 = {
        'USER': 'root',
    ...
    posted_dbOp = DB_Singleton_DEFAULT()     # 连接上传过的的数据的数据库
    ...
    if(videoInfoList):
        ...
        for item in videoInfoList:
            if(check_hasvideoindb1(posted_dbOp, checkDate_time, item)):
                ...
                try:
                    # 视频数据发布时间在当天
                    Downloader.download_video(urlpath=item[2], name=str(i), dstDirPath=setting['videoDirPath'], headers_=vid_headers)
                    checkIfSuccess = True
