...
class TaskTimer_AutoDealwithPost(Base_Timer):
    def task(self):
        ...
        by_classification = self.timerConfig.get("by_classification") if self.timerConfig.get("by_classification")==False else True
        if (self.timerConfig["task_type"] == 'keyParagraph'):
            ...
            t.run(table_name='tb_key_paragraph', classification=datapool, by_classification=by_classification)
            ...
            t_rela.run(table_name='tb_relative_paragraph', classification=datapool, by_classification=by_classification)
            ...
            t_comment.run(table_name='tb_comment', classification=datapool, by_classification=by_classification)
            ...
            t_articles.run(table_name='tb_article', classification=datapool, by_classification=by_classification)
            ...
            t_articles.env_config = self.timerConfig
            t_articles.run(table_name='tb_question', classification=datapool, by_classification=by_classification)
        else:
