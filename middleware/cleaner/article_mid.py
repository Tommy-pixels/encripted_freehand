...
class ArticleMiddleware(StringMiddleware):
    ...
    def clean_title(self, title):
        ...
        for symple in symple_lis:
            if (symple in title):
                title = self.clean_before_symple(title, symple=symple, del_direction='left')
        for originName in origin_lis:
            if (originName in title):
                title = title.replace(originName, '')
        res = self.clean_content_brackets(title, delAll=True)
    ...
    def clean_content(self, content):
        ...
        for origin in origin_lis:
            content = self.clean_between(content, s_left='(' + origin, s_right=')')  # 正则 直接小括号会报错
            content = self.clean_between(content, s_left='（' + origin, s_right='）')
            content = self.clean_between(content, s_left=origin, s_right='讯')
